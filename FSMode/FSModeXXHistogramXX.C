...
FSHistogramInfo*
FSModeHistogram::getHistogramInfo(TString index, bool cache){
  if (FSHistogram::m_histogramInfoCache.find(index) != FSHistogram::m_histogramInfoCache.end())
    return FSHistogram::m_histogramInfoCache[index];
  vector<TString> expandedIndices = expandIndices(index);
  return FSHistogram::getHistogramInfo(index,cache,expandedIndices);
}


vector<TString>
FSModeHistogram::expandIndices(TString index){
...
FSModeHistogram::getTH1FContents(TString fileName, TString ntName, TString category, 
  ...
  TString index =
    FSHistogram::getHistogramIndex(1,fileName,ntName,variable,bounds,cuts,scale,true,category);
...
FSModeHistogram::getTH2FContents(TString fileName, TString ntName, TString category, 
       ...
       vector< pair<TString,TString> > extraTreeContents){
  TString index =
    FSHistogram::getHistogramIndex(2,fileName,ntName,variable,bounds,cuts,scale,true,category);
  return getHistogramInfo(index)->getTHNFContents(NULL,extraTreeContents);
}
...
FSModeHistogram::getTHNF(int dimension,
  ...
  if (fsCuts.size() > 1){
    ...
    for (unsigned int i = 0; i < fsCuts.size(); i++){
      ...
      pair<TH1F*,TH2F*> histPair = FSModeHistogram::getTHNF(dimension, fileName, ntName,
                       category, variable, bounds, cuts_i, scale_i, histTree,
                       extraTreeContents);
      if (dimension == 1){
  ...
  for (unsigned int i = 0; i < modeVector.size(); i++){
    ...
    for (unsigned int j = 0; j < combinatorics.size(); j++){
      ...
      if (dimension == 1){
        TString index = FSHistogram::getHistogramIndex(dimension,fileName_i,ntName_i,variable_j,
                                                         bounds,cuts_j,scale);
        bool usedIndex = false;
...
FSModeHistogram::drawTH1F(TString fileName, TString ntName, TString category,
                        TString variable, TString bounds,
                        TString cuts, double scale, 
                        TString options, TCanvas* c1){
  drawTHNF(1,fileName,ntName,category,variable,bounds,cuts,scale,options,c1);
}
...
FSModeHistogram::drawTH2F(TString fileName, TString ntName, TString category,
                        ...
                        TString cuts, double scale,
                        TString options, TCanvas* c1){
  drawTHNF(2,fileName,ntName,category,variable,bounds,cuts,scale,options,c1);
}
...
FSModeHistogram::drawMCComponents(TString fileName, TString ntName, 
                                TString category, TString variable, 
                                TString bounds, TString cuts, double scale, TCanvas* c1){
